...
<li class="nav-item"><a class="nav-item" <? if (!empty(get_option('doliconnectbeta'))) { ?> data-bs-toggle="offcanvas" href="#offcanvasCartDoliconnect" role="button" aria-controls="offcanvasCartDoliconnect" <? } else { ?> href="<?php echo doliconnecturl('dolicart'); ?>" <? } ?> title="<?php _e('Basket', 'ptibogxivtheme'); ?>"><span class="fa-layers fa-fw fa-2x"><i class="fas fa-shopping-bag"></i><span class="fa-layers-counter fa-lg" id="DoliHeaderCarItems" style="background:Tomato"><?php echo (!empty(doliconnector( null, 'fk_order_nb_item'))?doliconnector( null, 'fk_order_nb_item'):'0'); ?></span></span></a></li>
<?php } 
